...
import sqlite3
try:
    con=sqlite3.connect('Students.db')    #Use try-except for event handling
    cursor=con.cursor()
...
while(i<10):
    try:
        name=input("Enter the name of the student: ")  #To input names
        marks=int(input('Enter the marks of the student: '))   #To input marks    
...
try:
    con=sqlite3.connect('Students.db')
    cursor=con.cursor()
    query='select * from students where marks>80'    #For students having marks>80
    cursor.execute(query)
